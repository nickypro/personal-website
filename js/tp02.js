function simpleBox(simpleText, extraText,) {
	...
	document.writeln(' <div> '   , simpleText );

	//create the sub text
	if (extraText != "") {
		document.writeln(' <div> ( ' ,  extraText ,'  ) </div> ');
	}
...
function multiBox(filePlace, fileName, fileNum, fileType, altText, extraDialogue) {
	...
	if (altText == -1) {
		...
		document.writeln(' <div> '   , altText );
	} else {
		document.writeln(' <div> '   , fileName, fileNum, fileType  );
	}

	//create the sub text
	if (extraDialogue != "" && extraDialogue != undefined)  {
		document.writeln(' <div> ( ' ,  extraDialogue ,'  ) </div> ');
	}
...
function listHomeworks( Directry, numberOfHomeworks, ArrayofDates, maximumSolution) {
	...
	document.writeln('<div id="', Directry, '" class="dropdownTable" style="display: none;">');
...
function listTutorials( Directry, numberOfHomeworks, maximumSolution, altTp, altTs) {
	...
	document.writeln('<div id="', Directry + 'tut', '" class="dropdownTable" style="display:none">');
...
function list( dropdownName, Directry, mainName, numberOfMains, altMainName, underTextDate, sideName, numberOfSides, altSideName) {
	document.writeln('<button class="dropButton" type="button" onclick="collapseItem(\'', Directry + 'tut','\')">  &#9660; ',dropdownName,'  </button>');
	
	document.writeln('<div id="', Directry + mainName, '" class="dropdownTable" style="display:none">');
	
	if (altMainName == undefined || altMainName == undefined) { altMainName = mainName};
	if (altSideName == undefined || altSideName == undefined) { altSideName = sideName};

	for (i = 1; i <= numberOfMains; i++) 
	{
		multiBox(Directry, mainName, i, '.pdf', altMainName, underTextDate[i-1]);
		if (i <= numberOfSides) 
		{
		multiBox(Directry, sideName, i, '.pdf', -1			, altSideName);
		}
